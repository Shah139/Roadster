...
public class ApiService {
    ...
    public static List<Map<String, Object>> fetchAreaCrimeRatesByDistrict(String district) throws Exception {
        try {
            String apiUrl = "http://localhost:8080/api/" + "crime-rates/" + district;
            HttpClient client = HttpClient.newHttpClient();
            HttpRequest request = HttpRequest.newBuilder()
                    .uri(URI.create(apiUrl))
                    .header("ngrok-skip-browser-warning", "true")
    ...
    public static List<Map<String, Object>> fetchAreaCongestionByDistrict(String district) throws Exception {
        try {
            String apiUrl = "http://localhost:8080/api/" + "area-congestion/" + district;
            HttpClient client = HttpClient.newHttpClient();
            HttpRequest request = HttpRequest.newBuilder()
                    .uri(URI.create(apiUrl))
                    .header("ngrok-skip-browser-warning", "true")
    ...
    /**
     * Submits a new report to the backend API
     * @param reportType The type/category of the incident (maps to report_type in backend)
     * @param description The detailed description of the incident
     * @param status The status of the report (PENDING, IN_PROGRESS, RESOLVED, REJECTED)
     * @param locationName The name of the location where the incident occurred
     * @param district The district name
     * @return The created Report object from the server response
     * @throws Exception if the submission fails
     */
    public static Report submitReport(String reportType, String description, String status,
                                    String locationName, String district) throws Exception {
        String apiUrl = "http://localhost:8080/api/reports";
        
        // Debug: Print the base URL
        System.out.println("[DEBUG ApiService] Base API URL: " + apiUrl);
        System.out.println("[DEBUG ApiService] Submitting to local Spring Boot backend");
        
        // Create the report object with all required fields
        Report newReport = new Report();
        // Set all 5 Report entity fields
        newReport.setReportType(reportType);  // Maps to report_type in database
        newReport.setDescription(description); // Maps to description in database
        newReport.setTimestamp(new Timestamp(System.currentTimeMillis())); // Maps to timestamp in database
        newReport.setStatus(status);  // Maps to status in database (from user input)
        // reportId will be auto-generated by database
        
        // Debug: Print complete report object details
        System.out.println("[DEBUG ApiService] Report object created with all fields:");
        System.out.println("  - ReportType: " + newReport.getReportType());
        System.out.println("  - Description: " + newReport.getDescription());
        System.out.println("  - Timestamp: " + newReport.getTimestamp());
        System.out.println("  - Status: " + newReport.getStatus());
        System.out.println("  - ReportId: " + newReport.getReportId() + " (will be auto-generated)");
        
        // Create JSON payload for the report
        ObjectMapper mapper = new ObjectMapper();
        String reportJson = mapper.writeValueAsString(newReport);
        
        // Debug: Print the JSON payload
        System.out.println("[DEBUG ApiService] JSON Payload: " + reportJson);
        
        // Build the request URL with query parameters as expected by Spring Boot
        String requestUrl = apiUrl + "?locationName=" + java.net.URLEncoder.encode(locationName, "UTF-8") + 
                           "&district=" + java.net.URLEncoder.encode(district, "UTF-8");
        
        // Debug: Print the full request URL
        System.out.println("[DEBUG ApiService] Full Request URL: " + requestUrl);
        System.out.println("[DEBUG ApiService] Query Parameters:");
        ...
        HttpRequest request = HttpRequest.newBuilder()
                ...
                // Remove ngrok header since we're using localhost
                .POST(HttpRequest.BodyPublishers.ofString(reportJson))
                .build();

        // Debug: Print request details
        System.out.println("[DEBUG ApiService] Making POST request to: " + request.uri());
        System.out.println("[DEBUG ApiService] Content-Type: " + request.headers().firstValue("Content-Type").orElse("not set"));
        
        HttpResponse<String> response;
        try {
            System.out.println("[DEBUG ApiService] Sending request to Spring Boot backend...");
            response = client.send(request, HttpResponse.BodyHandlers.ofString());
            System.out.println("[DEBUG ApiService] Request completed successfully");
        } catch (java.net.ConnectException ce) {
            throw new Exception("Cannot connect to Spring Boot backend at localhost:8080. " +
                               "Please make sure your Spring Boot application is running.\n" +
                               "Error: " + ce.getMessage());
        } catch (Exception e) {
            throw new Exception("Network error while connecting to backend: " + e.getMessage());
        }
        
        // Debug: Print response details
        System.out.println("[DEBUG ApiService] Response Status: " + response.statusCode());
        System.out.println("[DEBUG ApiService] Response Body: " + response.body());
        System.out.println("[DEBUG ApiService] Response Headers: " + response.headers().map());
        
        if (response.statusCode() == 201) {
            // Successfully created, parse the response
            System.out.println("[DEBUG ApiService] Success! Parsing response...");
            try {
                return mapper.readValue(response.body(), Report.class);
            } catch (Exception parseEx) {
                System.out.println("[DEBUG ApiService] Failed to parse response: " + parseEx.getMessage());
                throw new Exception("Report submitted successfully but failed to parse response: " + parseEx.getMessage());
            }
        } else if (response.statusCode() == 500) {
            // Internal server error - provide detailed debugging
            String errorMsg = "Internal Server Error (500) - Check your Spring Boot backend logs.\n" +
                             "Possible causes:\n" +
                             "- Database connection issues\n" +
                             "- Missing required fields in Report entity\n" +
                             "- SQL constraint violations\n" +
                             "- Service layer exceptions\n\n" +
                             "Server response: " + response.body();
            System.out.println("[DEBUG ApiService] 500 Error Details: " + errorMsg);
            throw new Exception(errorMsg);
        } else if (response.statusCode() == 404) {
            String errorMsg = "Endpoint not found (404) - Check if Spring Boot is running on localhost:8080\n" +
                             "Expected endpoint: POST /api/reports\n" +
                             "Server response: " + response.body();
            System.out.println("[DEBUG ApiService] 404 Error: " + errorMsg);
            throw new Exception(errorMsg);
        } else {
            String errorMsg = "Failed to submit report. Server responded with status: " + 
                              response.statusCode() + " - " + response.body();
            System.out.println("[DEBUG ApiService] Error: " + errorMsg);
            throw new Exception(errorMsg);
        }
    }
}